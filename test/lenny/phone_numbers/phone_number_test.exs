defmodule Lenny.PhoneNumberTest do
  ...
  describe "validation" do
    ...
    test "north american numbers don't need the +1" do
      assert PhoneNumber.changeset(%PhoneNumber{}, %{phone: "5551112222"}).valid?
    end

    test "north american numbers can have the +1" do
      assert PhoneNumber.changeset(%PhoneNumber{}, %{phone: "+15552221234"}).valid?
    end

    test "digits after the + can be as few as 6" do
      refute PhoneNumber.changeset(%PhoneNumber{}, %{phone: "+112345"}).valid?
      assert PhoneNumber.changeset(%PhoneNumber{}, %{phone: "+1123456"}).valid?
    end

    test "digits after the + can be as many as 15" do
      assert PhoneNumber.changeset(%PhoneNumber{}, %{phone: "+123456789012345"}).valid?
      refute PhoneNumber.changeset(%PhoneNumber{}, %{phone: "+1234567890123456"}).valid?
    end
  end
  ...
  test "inserting a 10 digit number without a country code adds the code on insertion" do
    phone_number =
      ...
      |> PhoneNumber.changeset(%{phone: "2223334444"})
      |> Repo.insert!()
  ...
  test "numbers with country codes are inserted un-modified" do
    phone_number =
      %PhoneNumber{user: user_fixture()}
      |> PhoneNumber.changeset(%{phone: "+1123456"})
      |> Repo.insert!()
    ...
    phone_number =
      %PhoneNumber{user: user_fixture()}
      |> PhoneNumber.changeset(%{phone: "+123456789012345"})
      |> Repo.insert!()
